/**
 * High-Frequency Oscillatory Ventilation (HFO) Approach
 * NICU Mechanical Ventilation Protocol
 * 
 * Based on clinical protocols for neonatal HFO management
 */

import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";

const HFOApproach = ({ weight, gestationalAge }) => {
  const w = parseFloat(weight) || 0;
  const ga = parseFloat(gestationalAge) || 0;

  return (
    <Card data-testid="hfo-approach">
      <CardHeader className="pb-2">
        <CardTitle className="text-base">High-Frequency Oscillatory Ventilation (HFO)</CardTitle>
        <CardDescription className="text-xs">Mechanical Ventilation Protocol</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="p-2 bg-white dark:bg-gray-800 rounded">
              ...
              <p className="text-muted-foreground">
                {ga < 37 ? "10-13 Hz (Preterm)" : ga < 42 ? "8-10 Hz (Term)" : "6-8 Hz (Older)"}
              </p>
            </div>
            <div className="p-2 bg-white dark:bg-gray-800 rounded">
        ...
        {/* Quick Reference - Frequency by Age */}
