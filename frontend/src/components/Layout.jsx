...
const SidePanel = ({ isOpen, onClose, theme, toggleTheme }) => {
  ...
  return (
    <>
      ...
      {/* Side Panel */}
      <div 
        className={`fixed top-0 left-0 h-full w-72 bg-white dark:bg-gray-900 z-50 transform transition-transform duration-300 shadow-xl ${
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'gradient-bg-dark' : 'gradient-bg-light'}`}>
      ...
      <button
        ...
        className="fixed top-4 left-4 z-[60] w-10 h-10 rounded-xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg flex items-center justify-center hover:bg-white dark:hover:bg-gray-800 transition-colors"
      >
